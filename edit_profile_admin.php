...
<!--tooltip------>
 ...
 $updatePro = "UPDATE `user` SET    `protectEmail` = '$protectEmail' WHERE `user_id`='$mid'";
       ...
       // $email = $_SESSION['email'];
        $listdetails = "SELECT * FROM `user` WHERE user_id='$mid'";
        ...
        while ($listdetails_query_fetch = mysql_fetch_array($listdetails_query)) {
            ...
            @$first = $listdetails_query_fetch['first'];
            @$middle = $listdetails_query_fetch['middle'];
            @$last = $listdetails_query_fetch['last'];
        ...
        if (isset($_POST['fname'])) {
            ...
            $checkexist = "SELECT * FROM `user` WHERE (email='$email' or phone='$phone') and user_id !='$mid'";
            ...
            if ($checkexist_check >= 1) {
                while ($checkexist_result1 = mysql_fetch_array($checkexist_query)) {
                    ...
                    if ($exemail == $email) {
                        ...
                        <?php
                          header("Location:edit_profile_admin.php?eid=$mid&fail=2");
                    } else {
                        ?>
                        <p class="notify_red">The phone number already exists. Please use different ones.</p>
                        <?php
                         header("Location:edit_profile_admin.php?eid=$mid&fail=1");
                        break;
                    }
                }
            } else { 
                $tm = time();
                $userupdate = "UPDATE `user` SET `first` = '$fname', `middle` = '$mname', `last` = '$lname', `status_time` = '$tm',`notify_time` = '$notify',`email` = '$email', `phone` = '$phone', `availability` = '$availability', `job` = '$typeofjob', `technology` = '$technologies', `salary` = '$salary', `zip` = '$zip', `distance` = '$zipdist', `flexible` = '$flexible', `country` = '$country' WHERE `user_id`='$mid'";
                ...
                $permissions = str_replace(",,", ",", $permissions);
                $addper = "UPDATE `permissions` SET `permissions` = '$permissions' where user_id='$mid'";
                $adduser_per = mysql_query($addper);
				if($listdetails_query_fetch['availability'] != $availability){
					$userstatus = "UPDATE `user` SET  `todaystatus` = '1' WHERE `user_id`='$mid'";
                ...
                header("Location:edit_profile_admin.php?eid=$mid&succ=1&#stop");
                ?>
        ...
        ?>
            ...
            <?php } ?>
            <?php if ($_GET['fail']==1) { ?>
			  <p class="notify_green">The phone number already exists. Please use different ones.</p>
            <?php } ?>
            <?php if ($_GET['fail']==2) { ?>
			  <p class="notify_green">The email Address already exists. Please use different ones.</p>
            <?php } ?>
            <?php if (@$role == 2) { ?>
									  
									 
							<div style="clear:both;margin-top:0px;float:right">
			...
			<button type="submit" class="btn btn-info active tooltip1" name="protect"  <?php  if ($_POST['protect']=='Public' || $protectexisting=='Public'){  ?> title="You selected public then the information will be disclosed to recruiters" style="background-color: ActiveCaption;" disabled <?php }else{ ?> title="If you select public then the information will be disclosed to recruiters"<?php }?>value="Public"  ><i class="fa fa-unlock-alt"></i> Public</button>
			 ...
			 <div class="panel panel-default">
				...
				<form role="form" action="edit_profile_admin.php" name="checkval" id="checkval"  method="post"  onsubmit="return(validate());">
                                <table class="table table-hover tablebd">
                                      ...
                                      <tr>
                                            ...
                                            <input class="form-control" id="fname" oninvalid="fullname(this);" oninput="fullname(this);"   required="required" name="fname" value="<?php echo $first; ?>" placeholder="First Name (Mandatory)" >
                                        </div></td></tr>
                                         ...
                                         <tr>
                                            ...
                                            <input class="form-control" id="mname" name="mname" value="<?php echo @$middle; ?>" placeholder="Middle Name" >
                                        </div></td></tr>
										 <tr>
                                            ...
                                            <input class="form-control" id="lname" oninvalid="lastname(this);" oninput="lastname(this);"   required="required" name="lname" value="<?php echo @$last; ?>" placeholder="Last Name  (Mandatory)">
                                        </div></td></tr>
										<tr>
                    ...
                    <?php } ?>
										...
										<tr>
                                            ...
                                            <td>
											<?php
                    $row = mysql_fetch_assoc(mysql_query($sqlComp = "SELECT * FROM `permissions` where `user_id`='$mid' "));
                    $findme1 = '1';
                    $findme2 = '2';
                    $findme3 = '3';
                    $findme4 = '4';
                    ...
                    <?php } ?>
                                    ...
                                    <tbody>
										...
										<tr>
                                            ...
                                            <input class="form-control" id="email"  style="float:left;"  oninvalid="emailid(this);"  oninput="emailid(this);"   required="required" name="email" value="<?php echo @$email; ?>" placeholder="Email Id (Mandatory)" />
											 <?php if($protectexisting=='Public'){ if ($_GET['eml']=='1' || $protectexistingEmail=='1' || $protectexistingEmail==''){  ?><a href="edit_profile_admin.php?eid=<?php echo $_GET['eid'];?>&eml=0&#follow"  class="tooltip" style="opacity:1;margin-left: 400px;" title="Your email is not disclosed if click show will be disclosed to recruiters">show</a><?php }else if ($_GET['eml']=='0' || $protectexistingEmail=='0' || $protectexistingEmail==''){  ?><a href="edit_profile_admin.php?eid=<?php echo $_GET['eid'];?>&eml=1&#follow"  class="tooltip" style="opacity:1;margin-left: 400px;" title="Your email is disclosed if click Hide will not disclosed to recruiters">Hide</a><?php } } ?>  
                                        </div>
										</td>
										<td>
                                            ...
                                            <input class="form-control" id="phone" style="float:left;" oninvalid="phonenumber(this);" oninput="phonenumber(this);"   required="required" name="phone" value="<?php echo @$phone; ?>" placeholder="Your Mobile Number(Mandatory)" /> <?php if($protectexisting=='Public'){ if ($_GET['tph']=='1' || $protectexistingPhone=='1' || $protectexistingPhone==''){  ?><a href="edit_profile_admin.php?eid=<?php echo $_GET['eid'];?>&tph=0&#follow"  class="tooltip"  style="opacity:1;margin-left: 400px;" title="Your phone is not disclosed if click show will be disclosed to recruiters" > show </a><?php } else if ($_GET['tph']=='0' || $protectexistingPhone=='0' || $protectexistingPhone==''){  ?><a href="edit_profile_admin.php?eid=<?php echo $_GET['eid'];?>&tph=1&#follow" class="tooltip" style="opacity:1;margin-left: 400px;" title="Your Phone is disclosed if click Hide will not disclosed to recruiters">   Hide </a><?php } } ?>
                                        </div></td></tr>
