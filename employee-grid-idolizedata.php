...
if (strlen($requestData['refineSearch']) > 1 && strlen($requestData['nameSearch']) > 1) {
    ...
    if ($semail == 0) {
        ...
        if($refineSearch =='No Status'){
          $sql.=" FROM user where  role=2 and `first` LIKE  '%{$requestData['nameSearch']}%' and (`availability` = '$refineSearch' OR availability='') and idolize=1 ";   
        }else{
         $sql.=" FROM user where  role=2 and `first` LIKE  '%{$requestData['nameSearch']}%' and `availability` = '$refineSearch' and idolize=1 ";    
        }
       
    } else if ($semail == 1) {
        $sql = "SELECT  * ";
         if($refineSearch =='No Status'){
         $sql.=" FROM user where  role=2 and  `email` LIKE  '%{$requestData['nameSearch']}%' and (`availability`  = '$refineSearch' OR availability='') and idolize=1 ";  
        }else{
         $sql.=" FROM user where  role=2 and  `email` LIKE  '%{$requestData['nameSearch']}%' and `availability`  = '$refineSearch' and idolize=1 ";
        }
    ...
    $sql = "SELECT  * ";
     if($refineSearch =='No Status'){
        $sql.= " FROM user where    (`availability` = '$refineSearch' OR availability='') and role=2 and idolize=1";
        }else{
        $sql.= " FROM user where  `availability` = '$refineSearch' and role=2  and idolize=1";
        }
    
} else if (strlen($requestData['nameSearch']) > 1) {
    if ($semail == 0) {
        $sql = "SELECT  * ";
        $sql.=" FROM user where  role=2 and `first` LIKE  '%{$requestData['nameSearch']}%' and idolize=1 ";
    } else if ($semail == 1) {
        $sql = "SELECT  * ";
        $sql.=" FROM user where  role=2 and  `email` LIKE  '%{$requestData['nameSearch']}%' and idolize=1";
    }
} else {
    $sql = "SELECT  * ";
    $sql.=" FROM user where  role=2 and idolize=1";
}
