...
class Filetotext {
    ...
    public function convertToText() {
     ...
     public function convertToText1() {
        ...
        if ($file_ext == "doc" || $file_ext == "docx") {
            if ($file_ext == "doc") {
                return $this->read_doc1();
            } else {
                return $this->read_docx1();
            }
        } else if ($file_ext == "pdf") {
            $this->setFilename($this->filename);
            $this->decodePDF1();
    ...
    //this below comented read_doc() is previous function
...
//		return $outtext;
//	}
    //this above comented read_doc() is previous function
    private function read_doc1() {
        if (file_exists($this->filename)) {
            if (($fh = fopen($this->filename, 'r')) !== false) {
                ...
                if (strlen($textLength) <= 8) {
                    $extracted_plaintext = fread($fh, $textLength);
                } else {
                    $extracted_plaintext = 'empty';
                }
...
//        $content = str_replace('</w:t></w:r>', "\r\n", $content);
        $striped_content = strip_tags($content);
// echo $striped_content;exit;
        return $striped_content;
    }
    private function read_docx1() {
